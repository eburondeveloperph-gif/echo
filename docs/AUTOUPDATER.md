...
Eburon Echo includes automatic updates powered by Tauri's updater plugin. This document explains how it works for both users and developers.
...
```bash
cd tauri && bun tauri signer generate -w ~/.tauri/eburon-echo.key
```

This creates:
- **Private key**: `~/.tauri/eburon-echo.key` (keep this secret!)
- **Public key**: `~/.tauri/eburon-echo.key.pub`

## 2. Update Configuration

Copy the content from `~/.tauri/eburon-echo.key.pub` and replace the placeholder in `tauri/src-tauri/tauri.conf.json`:
...
```json
{
  "plugins": {
    "updater": {
      ...
      "endpoints": [
        "https://github.com/YOUR_USERNAME/eburon-echo/releases/latest/download/latest.json"
      ]
    }
  }
}
...
**macOS/Linux:**
```bash
export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/eburon-echo.key)"
...
**Windows PowerShell:**
```powershell
$env:TAURI_SIGNING_PRIVATE_KEY = Get-Content ~/.tauri/eburon-echo.key -Raw
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": {
      ...
      "url": "https://github.com/YOUR_USERNAME/eburon-echo/releases/download/v0.2.0/eburon-echo_0.2.0_aarch64.dmg"
    },
    "darwin-x86_64": {
      "signature": "CONTENT_FROM_.app.tar.gz.sig",
      "url": "https://github.com/YOUR_USERNAME/eburon-echo/releases/download/v0.2.0/eburon-echo_0.2.0_x64.dmg"
    },
    "linux-x86_64": {
      "signature": "CONTENT_FROM_.AppImage.sig",
      "url": "https://github.com/YOUR_USERNAME/eburon-echo/releases/download/v0.2.0/eburon-echo_0.2.0_amd64.AppImage"
    },
    "windows-x86_64": {
      "signature": "CONTENT_FROM_.msi.sig",
      "url": "https://github.com/YOUR_USERNAME/eburon-echo/releases/download/v0.2.0/eburon-echo_0.2.0_x64_en-US.msi"
    }
  }
}
```
...
- Add `TAURI_SIGNING_PRIVATE_KEY` with the content of `~/.tauri/eburon-echo.key`
