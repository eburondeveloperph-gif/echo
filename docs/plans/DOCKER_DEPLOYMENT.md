...
Docker support makes Eburon Echo easier to deploy, especially for:
...
```bash
# CPU-only version
docker run -p 8000:8000 -v eburon-echo-data:/app/data \
  ghcr.io/jamiepine/eburon-echo:latest

# NVIDIA GPU version
docker run --gpus all -p 8000:8000 -v eburon-echo-data:/app/data \
  ghcr.io/jamiepine/eburon-echo:latest-cuda

# AMD GPU version (experimental)
docker run --device=/dev/kfd --device=/dev/dri -p 8000:8000 \
  -v eburon-echo-data:/app/data \
  ghcr.io/jamiepine/eburon-echo:latest-rocm
```
...
services:
  eburon-echo:
    image: ghcr.io/jamiepine/eburon-echo:latest-cuda
    ports:
      - "8000:8000"
    volumes:
      - eburon-echo-data:/app/data
...
Build and run:
```bash
docker build -t eburon-echo .
docker run -p 8000:8000 -v $(pwd)/data:/app/data eburon-echo
```
...
Build:
```bash
docker build -f Dockerfile.optimized -t eburon-echo:slim .
```
...
docker run --device=/dev/kfd --device=/dev/dri \
  ...
  -p 8000:8000 -v eburon-echo-data:/app/data \
  eburon-echo:rocm
```
...
```bash
docker run -v eburon-echo-data:/app/data \           # Profiles, generations, history
           -v huggingface-cache:/root/.cache/huggingface \  # Downloaded models
           -p 8000:8000 eburon-echo
```
...
Configure Eburon Echo via environment variables:
...
doctl compute droplet create eburon-echo \
...
sh get-docker.sh
docker run --gpus all -d -p 80:8000 eburon-echo:cuda
```
...
```bash
# Build and push
docker build -t gcr.io/your-project/eburon-echo .
docker push gcr.io/your-project/eburon-echo

# Deploy to Cloud Run
gcloud run deploy eburon-echo \
  --image gcr.io/your-project/eburon-echo \
  --platform managed \
  --region us-central1 \
  --memory 4Gi \
  --cpu 2 \
...
[build]
  image = "ghcr.io/jamiepine/eburon-echo:latest"
...
```bash
# Fix permissions
docker run --user $(id -u):$(id -g) -v $(pwd)/data:/app/data eburon-echo
```
...
```bash
docker run -e GPU_MEMORY_FRACTION=0.5 eburon-echo
```

Or use CPU-only:
```bash
docker run -e DEVICE=cpu eburon-echo
```
...
```bash
docker run -v huggingface-cache:/root/.cache/huggingface eburon-echo
```

Or use host cache:
```bash
docker run -v ~/.cache/huggingface:/root/.cache/huggingface eburon-echo
```
...
Change host port:
```bash
docker run -p 8080:8000 eburon-echo  # Use port 8080 instead
```
...
```dockerfile
RUN useradd -m -u 1000 eburon-echo
USER eburon-echo
```
...
docker run -e GPU_MEMORY_FRACTION=0.8 eburon-echo

# Allow GPU memory growth (prevents OOM)
docker run -e TF_FORCE_GPU_ALLOW_GROWTH=true eburon-echo
```
...
View container logs:
```bash
docker logs -f eburon-echo

# Or with compose
docker compose logs -f eburon-echo
```
...
4. Report issues: [GitHub Issues](https://github.com/jamiepine/eburon-echo/issues)
