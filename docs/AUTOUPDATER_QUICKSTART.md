...
This creates:
- Private key: `~/.tauri/eburon-echo.key` (keep secret!)
- Public key: `~/.tauri/eburon-echo.key.pub` (safe to share)

### 2. Update Tauri Config

Open `tauri/src-tauri/tauri.conf.json` and:

1. Replace `"REPLACE_WITH_YOUR_PUBLIC_KEY"` with the content from `~/.tauri/eburon-echo.key.pub`
...
- Make sure you copied the entire content from `eburon-echo.key.pub`
...
- Verify private key file exists at `~/.tauri/eburon-echo.key`
