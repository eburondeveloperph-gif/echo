...
function createPlaceholderBinary(targetTriple) {
  ...
  const binaryName = `eburon-echo-server-${targetTriple}${isWindows ? '.exe' : ''}`;
  ...
  if (isWindows) {
    ...
    const script = `#!/bin/sh
echo "[eburon-echo-server] Dev mode placeholder - start the real server with: bun run dev:server"
exit 1
`;
